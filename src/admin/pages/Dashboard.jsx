...
/* Each block appears only for the roles the server sends it to: an agent's
   overview carries no submissions, an editor's no clients. */
  ...
  return (
    <div className="adm-page wide">
      <header className="adm-head">
        <div>
          <span className="adm-eyebrow">{roleLabel(o.role)}</span>
          <h1>{greeting()}</h1>
        </div>
      ...
      {caps.includes('messages') && !o.mail_ready?.resend && (
        <div className="adm-alert">
          <b>Email is not set up.</b> Form submissions are still saved here, but no one is emailed about them until the Resend key is in the server config.
        </div>
      )}
      {caps.includes('messages') && o.mail_ready && !o.mail_ready.inbound && (
      ...
      {caps.includes('boards') && (
        <section>
          <h2 className="adm-sub">{o.role === 'agent' ? 'Your clients' : 'Boards'}</h2>
          <div className="adm-tiles">
            {(o.boards || []).map((b) => (
              <Link key={b.id} className="adm-tile" to={`/admin/boards/${b.id}`}>
                <b>{b.items}</b>
                <span>{b.name}</span>
                <small>{o.role === 'agent' ? 'assigned to you' : 'clients'}</small>
              </Link>
            ))}
            {caps.includes('configure') && (
              <button type="button" className="adm-tile adm-tile--add" onClick={newBoard}>
                <b>+</b>
                <span>New board</span>
                <small>for a new kind of file</small>
              </button>
            )}
          </div>
          {(o.boards || []).length === 0 && !caps.includes('configure') && <p className="adm-dim adm-pad">No boards yet — an administrator sets them up.</p>}
        </section>
      )}

      {caps.includes('messages') && (
        <>
          <h2 className="adm-sub">From the website</h2>
          <div className="adm-tiles">
          ...
          {o.mail_failed_7d > 0 && (
            <p className="adm-err" style={{ marginTop: 16 }}>
              {o.mail_failed_7d} email{o.mail_failed_7d > 1 ? 's were' : ' was'} not delivered in the last 7 days — <Link to="/admin/email">see the email log</Link>.
            </p>
          )}
        </>
      )}
